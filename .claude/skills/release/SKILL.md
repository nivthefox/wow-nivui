...
- `$version` (optional) — The version to release (e.g., `2.1.0`). If omitted, auto-increment the current version's last numeric segment (e.g., `2.0.1` → `2.0.2`).

## Steps

### 1. Determine the version

- Read `NivUI.toc` and extract the current `## Version:` value.
...
- Stage `CHANGELOG.md` and `NivUI.toc`.
